...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE) {
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, array('id_usuario' => $id));
		} else {
			$data = array(
				'activation_code' => NULL,
				'active' => 1
	...
	public function reset_password($identity, $new)
	{
		...
		$query = $this->db->select('id_usuario, password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		$old_password_matches = $this->hash_password_db($user->id_usuario, $old);
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['groups'], array('nome' => $this->config->item('default_group', 'ion_auth')), 1)->row();
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id_usuario, password, active, last_login, cargo_id, funcionario_id')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id_usuario', 'desc')
			->get($this->tables['users']);
		...
		if ($query->num_rows() === 1) {
			...
			$password = $this->hash_password_db($user->id_usuario, $password);
			...
			if ($password === TRUE) {
				...
				$fun = $this->fm->GetById('id_funcionario', $user->funcionario_id);
				$ent = $this->em->GetById('id_entidade', $fun['entidade_id']);
				$cargo = $this->cm->GetById('id_cargo', $user->cargo_id);
				...
				$login = array(
					...
					'so' => ($so->isMobile()) ? $so->getName() . " / Mobile" : $so->getName(),
				...
				$this->update_last_login($user->id_usuario);

				$this->clear_login_attempts($identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth')) {
					$this->remember_user($user->id_usuario);
				}
			...
			if ($last_login + $recheck < time()) {
				...
				if ($query->num_rows() === 1) {
					...
					if (substr(CI_VERSION, 0, 1) == '2') {
						$this->session->unset_userdata(array($identity => '', 'id_usuario' => '', 'user_id' => ''));
					} else {
						$this->session->unset_userdata(array($identity, 'id_usuario', 'user_id'));
					}
					return FALSE;
	...
	public function users($groups = NULL)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			$this->db->select(array(
				$this->tables['users'] . '.*',
				$this->tables['users'] . '.id_usuario as id',
				$this->tables['users'] . '.id_usuario as user_id'
			));
		}
		...
		if (isset($groups)) {
			...
			if (isset($groups) && !empty($groups)) {
				...
				$this->db->join(
					...
					$this->tables['users_groups'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.id_usuario',
					'inner'
				);
			}
			...
			if (!empty($group_names)) {
				$this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id_cargo', 'inner');
				$this->db->where_in($this->tables['groups'] . '.name', $group_names);
			}
			if (!empty($group_ids)) {
				$this->db->{$or_where_in}($this->tables['users_groups'] . '.id_cargo', $group_ids);
			}
		}
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.nome, ' . $this->tables['groups'] . '.descricao')
			->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
			->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id_cargo')
	...
	public function group($id = NULL)
	{
		...
		if (isset($id)) {
			$this->where($this->tables['groups'] . '.cargo_id', $id);
		}

		$this->limit(1);
		$this->order_by('cargo_id', 'desc');
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('id_usuario' => $user->id_usuario));
	...
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['users'], array('id_usuario' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('id_usuario' => $id));
	...
	public function set_session($user, $fun, $ent, $cargo)
	{
		...
		$session_data = array(
			...
			'email' => $user->email,
			'user_id' => $user->id_usuario, //everyone likes to overwrite id so we'll use user_id
			'old_last_login' => $user->last_login,
			'last_check' => time(),
			'idfuncionario' => $fun['id_funcionario'],
			'user_nome' => $fun['nome'],
			'user_cpf' => $fun['cpf'],
			'entidade_id' => $ent['id_entidade'],
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('remember_code' => $salt), array('id_usuario' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id_usuario, email, last_login')
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['groups'], array('nome' => $group_name))->num_rows();
		if ($existing_group !== 0) {
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name)) {
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['groups'], array('nome' => $group_name))->row();
		...
		$group = $this->db->get_where($this->tables['groups'], array('id_cargo' => $group_id))->row();
		...
		$this->db->update($this->tables['groups'], $data, array('id_cargo' => $group_id));
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['groups'], array('id_cargo' => $group_id));
